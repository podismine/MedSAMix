...
class ViTLfsMultiMerge(MergeStrategy):
    ...
    def __init__(self, config):
        ...
        logger.info(f"current evaluate tasks {self.evaluate_tasks}")
    ...
    def objective(self, config, seed, budget):
        ...
        try:
            ...
            logger.info(f"current res is {res}")
    ...
    def eval_config(self, config, config_id=0, prefix = ''):
        ...
        try:
            ...
            try:
                with open(f"results/merge_res_{prefix}.txt", 'a') as f:
                    for key, val in res.items():
                        eval_result[key] = val
                        f.write(f"{prefix}_{key}: mean dice: {val:.3f} \n")
            except Exception as e:
                logger.error(f"writing res fail: {str(e)}")
                print(e)
            # Clean up
            # if os.path.exists(temp_model_path):
            #     import shutil
